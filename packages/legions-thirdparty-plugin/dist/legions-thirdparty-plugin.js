...
var legionsThirdpartyPlugin = (function (exports) {
	...
	*/
	...
	var proxyGetters = function proxyGetters(proxytarget, orginSource) {
	...
	function onLoadScript(plugin) {
	  ...
	  if (!window[PLUGINS[plugin.name]] && !window.parent[PLUGINS[plugin.name]] && !document.getElementById(id)) {
	    ...
	    script.onload = script.onreadystatechange = function () {
	      ...
	      if (!this.readyState || /^(loaded|complete)$/.test(this.readyState)) {
	        if (plugin.name === 'jsBarcode') {
	          LEGIONS_THIRDPARTY_PLUGIN[plugin.name] = window[PLUGINS[plugin.name]]['JsBarcode'];
	        } else if (plugin.name === 'dexie') {
	          LEGIONS_THIRDPARTY_PLUGIN[plugin.name] = window[PLUGINS[plugin.name]]['DexieUtils'];
	        } else {
	  ...
	  LegionsThirdpartyPlugin.prototype.subscribe = function (name, callback) {
	    if (typeof name === 'string') {
	      ...
	      name.map(function (item) {
	        list_1.push({
	          name: item,
	          value: LEGIONS_THIRDPARTY_PLUGIN[item]
	        });
	      });

	      if (list_1.every(function (item) {
	        return item.value;
	      })) {
	        callback();
	      } else {
