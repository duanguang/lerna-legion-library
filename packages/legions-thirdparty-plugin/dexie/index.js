...
(function (global, factory) {
	...
	***************************************************************************** */
	...
	function setProp(obj, prop, functionOrGetSet, options) {
	    defineProperty$2(obj, prop, extend(functionOrGetSet && hasOwn(functionOrGetSet, "get") && typeof functionOrGetSet.get === 'function' ?
	...
	function getPropertyDescriptor(obj, prop) {
	    var pd = getOwnPropertyDescriptor$2(obj, prop), proto;
	...
	*/
	...
	function () {
	  ...
	  DexieUtils.getInstanceDexie = function (key) {
	    var store = this.dexies.find(function (ws) {
	      return ws.key && ws.key === key;
	    });
	  ...
	  DexieUtils.registerInstanceDexie = function (store, key, schema) {
	    var _this = this;

	    var dbItem = this.getInstanceDexie(key);
	    ...
	    if (dbItem && dbItem.db) {
	      if (!dbItem.db.isOpen()) {
	        init(store).then(function (result) {
	          dbItem.db = result;
	          _this.dexies = _this.dexies.map(function (item) {
	      ...
	      if (!this.getInstanceDexie(key)) {
	        ...
	        init(store).then(function (result) {
	          if (_this.getInstanceDexie(key) && _this.getInstanceDexie(key).openState === 'pending') {
	            _this.dexies = _this.dexies.map(function (item) {
	              if (item.key === key) {
