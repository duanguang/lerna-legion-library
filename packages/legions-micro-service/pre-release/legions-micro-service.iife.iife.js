/**
  * legions-micro-service v0.0.10-beta.1
  * (c) 2021 duanguang
  * @license MIT
  */
var legionsMicroservice = (function (exports) {
	...
	***************************************************************************** */
	...
	var legionsImportHtmlEntry_umd_4 = legionsImportHtmlEntry_umd.fetch;
	...
	var processor;
	var process$5 = function process(appWrapper, stylesheetElement, appName) {
	      ...
	      if (event) {
	        ...
	        if (types && Array.isArray(types)) {
	          types.forEach(function (type) {
	            if (type === id) {
	              deps[id](cloneDeep_1(state), cloneDeep_1(prevState), event);
	            }
	          });
	        }
	      }
	...
	function getMicroAppStateActions(id, isMaster) {
	  return {
	     ...
	     */
	    onGlobalStateChange: function onGlobalStateChange(callback, options) {
	     ...
	     * @param state
	     */
	    setGlobalState: function setGlobalState(state, event) {
	      if (state === void 0) {
	        state = {};
	      }
	...
	process.env.NODE_ENV === 'development' ? ['__REACT_ERROR_OVERLAY_GLOBAL_HOOK__'] : [];
	...
	function createFakeWindow(global) {
	  ...
	  Object.getOwnPropertyNames(global) // 遍历出 window 对象所有不可配置属性
	  .filter(function (p) {
	    ...
	    if (descriptor) {
	      ...
	      if (hasGetter) propertiesWithGetter.set(p, true); // 将属性和描述设置到 fakeWindow 对象，并且冻结属性描述符，不然有可能会被更改，比如 zone.js
	...
	function convertLinkAsStyle(element, postProcess, fetchFn) {
	  if (fetchFn === void 0) {
	    fetchFn = legionsImportHtmlEntry_umd_4;
	  }
	            ...
	            if (scopedCSS) {
	              ...
	              if (linkElementUsingStylesheet) {
	                stylesheetElement = convertLinkAsStyle(element, function (styleElement) {
	                  return process$5(mountDOM_1, styleElement, appName_1);
	                }, legionsImportHtmlEntry_umd_4);
	                dynamicLinkAttachedInlineStyleMap.set(element, stylesheetElement);
	              } else {
	                process$5(mountDOM_1, stylesheetElement, appName_1);
	              }
	        ...
	        case SCRIPT_TAG_NAME:
	          {
	            var _b = element,
	                src = _b.src,
	                text = _b.text; // some script like jsonp maybe not support cors which should't use execScripts
	...
	function patchAtMounting(appName, elementGetter, sandbox, scopedCSS, excludeAssetFilter) {
	  ...
	  var patchersInSandbox = (_a = {}, _a[SandBoxType.Proxy] = __spread(basePatchers, [function () {
	    return patchStrictSandbox(appName, elementGetter, sandbox.sandbox, true, scopedCSS, excludeAssetFilter);
	  }]), _a[SandBoxType.Snapshot] = __spread(basePatchers, [function () {
	    return patchLooseSandbox(appName, elementGetter, sandbox.sandbox, true, scopedCSS, excludeAssetFilter);
	  }]), _a);
	...
	function patchAtBootstrapping(appName, elementGetter, sandbox, scopedCSS, excludeAssetFilter) {
	  ...
	  var patchersInSandbox = (_a = {}, _a[SandBoxType.Proxy] = [function () {
	    return patchStrictSandbox(appName, elementGetter, sandbox.sandbox, false, scopedCSS, excludeAssetFilter);
	  }], _a[SandBoxType.Snapshot] = [function () {
	    return patchLooseSandbox(appName, elementGetter, sandbox.sandbox, false, scopedCSS, excludeAssetFilter);
	  }], _a);
	  return (_b = patchersInSandbox[sandbox.type]) === null || _b === void 0 ? void 0 : _b.map(function (patch) {
	    return patch();
	  });
	...
	function createElement(appContent, strictStyleIsolation, scopedCSS, appName) {
	  ...
	  if (scopedCSS) {
	    ...
	    forEach_1(styleNodes, function (stylesheetElement) {
	      process$5(appElement, stylesheetElement, appName);
	    });
	  }
	    ...
	    return __generator(this, function (_m) {
	      switch (_m.label) {
	        case 0:
	          ...
	          markName = "[legions] App " + appInstanceId + " Loading"; //@ts-ignore
	        ...
	        case 4:
	          ...
	          if (sandbox) {
	            sandboxInstance = createSandbox(appName, initialAppWrapperGetter, scopedCSS, excludeAssetFilter); // 用沙箱的代理对象作为接下来使用的全局对象
	...
	var firstMountLogLabel = '[legions] first app mounted'; //@ts-ignore
