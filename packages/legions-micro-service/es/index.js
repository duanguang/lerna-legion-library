/**
  *  legions-micro-service v0.1.1
  * (c) 2021 duanguang
  * @license MIT
  */
import { importHTML, execScripts, fetch } from 'legions-import-html-entry';
import { getAppStatus, NOT_LOADED, getMountedApps, checkActivityFunctions, registerApplication, mountRootParcel, start as start$1, navigateToUrl } from 'single-spa';
export { addErrorHandler, getMountedApps, removeErrorHandler } from 'single-spa';
...
var microApps$2 = []; // {name:'',entry:'url',container:'DOMid',appId:'',styleId:'',loading:true}
...
var MicroApps$1 = /** @class */ (function () {
    ...
    MicroApps.prototype.isRegister = function (apps) {
        var unregisteredApps = microApps$2.filter(function (item) { return item.name === apps.name; });
    ...
    MicroApps.prototype.register = function (apps) {
        if (Array.isArray(apps)) {
            ...
            microApps$2 = __spreadArray(__spreadArray([], __read(microApps$2)), __read(unregisteredApps));
        }
    };
    MicroApps.prototype.addRender = function (app) {
        microApps$2.forEach(function (item) {
    ...
    MicroApps.prototype.mounted = function (apps) {
        ...
        var isCheckRegister = function () {
            ...
            if (!oldApps) {
                microApps$2 = __spreadArray(__spreadArray([], __read(microApps$2)), [apps]);
            }
            else {
                apps = oldApps[0];
            }
        ...
        function main() {
            ...
            if (scriptResources$1 && scriptResources$1.hasOwnProperty(sourceUrl)) {
                currentEnvironment = 'sandbox';
                apps.render && apps.render();
                renderStyles(scriptResources$1[sourceUrl]['styles']);
                return;
            }
                ...
                res.getExternalScripts().then(function (exports) {
                    if (res.getScripts) {
                        ...
                        excludeFiles_1.forEach(function (item) {
                            var fn = function () {
                                var p = new Promise(function (resolve, reject) {
                                    onloadScript(item.url, function () {
                                        scriptResources$1[sourceUrl].scriptCache.forEach(function (s) {
                                            if (s.key === item.url) {
                                                externalOnloadScript$1.push({
                                                    url: item.url,
                                                    code: s.code,
                                                });
                                                resolve(externalOnloadScript$1);
                                            }
                                        });
                                    });
                                });
                            ...
                            if (!externalOnloadScript$1.length) {
                                // 如果外部加载资源项为空，则直接加载资源
                                fn();
                            }
                            else {
                                externalOnloadScript$1.forEach(function (entry) {
                        ...
                        Promise.all(promiseLoadScript_1).then(function (values) {
                            ...
                            res.getExternalStyleSheets().then(function (exports) {
                                if (scriptResources$1[sourceUrl]['styles'].length) {
                                    scriptResources$1[sourceUrl]['styles'] = [];
                                }
                                exports.forEach(function (item) {
                                    scriptResources$1[sourceUrl]['styles'].push(item);
                                    var style = document.createElement('style');
                                    var styleWrap = document.getElementById(styleId);
                                    style.innerHTML = item;
                                    //@ts-ignore
...
var MicroApps = /** @class */ (function () {
    ...
    MicroApps.prototype.register = function (apps) {
        if (Array.isArray(apps)) {
            ...
            microApps$1 = __spreadArray(__spreadArray([], __read(microApps$1)), __read(unregisteredApps));
        }
    };
    MicroApps.prototype.bootstrap = function (apps, importHtmlentryResult) {
        ...
        var isCheckRegister = function () {
            var oldApps = _this.isRegister(apps);
            if (!oldApps) {
                microApps$1 = __spreadArray(__spreadArray([], __read(microApps$1)), [apps]);
                syncMicroApps();
            }
        };
...
function prefetch(entry, opts) {
    ...
    requestIdleCallback(function () { return __awaiter(_this, void 0, void 0, function () {
        ...
        return __generator(this, function (_b) {
            switch (_b.label) {
                case 0: return [4 /*yield*/, importHTML(entry, opts)];
                case 1:
...
function prefetchAfterFirstMounted(apps, opts) {
    window.addEventListener('single-spa:first-mount', function listener() {
        var notLoadedApps = apps.filter(function (app) { return getAppStatus(app.name) === NOT_LOADED; });
        //@ts-ignore
...
function getAddOn$1(global, publicPath) {
...
function getAddOns(global, publicPath) {
    return mergeWith({}, getAddOn(global), getAddOn$1(global, publicPath), function (v1, v2) { return concat(v1 !== null && v1 !== void 0 ? v1 : [], v2 !== null && v2 !== void 0 ? v2 : []); });
}
...
var events = {};
// 触发全局监听
function emitGlobal(state, prevState, event) {
    Object.keys(deps).forEach(function (id) {
        if (typeof deps[id] === 'function') {
            if (event) {
                var types = events[event.scope];
...
function getMicroAppStateActions(id, isMaster) {
    return {
         ...
         */
        onGlobalStateChange: function (callback, options) {
         ...
         * @param state
         */
        setGlobalState: function (state, event) {
            if (state === void 0) { state = {}; }
            if (state === globalState) {
                console.warn('[legions] state has not changed！');
                return false;
...
process.env.NODE_ENV === 'development'
    ...
    : [];
var variableWhiteList = __spreadArray([
    'System',
    '__cjsWrapper'
], __read(variableWhiteListInDev));
        ...
        if (descriptor) {
            ...
            if (p === 'top' ||
                ...
                (process.env.NODE_ENV === 'test' &&
                    (p === 'mockTop' || p === 'mockSafariTop'))) { // 将 top、parent、self、window 这几个属性由不可配置改为可配置
...
var ProxySandbox = /** @class */ (function () {
    ...
    ProxySandbox.prototype.inactive = function () {
        ...
        //@ts-ignore
        if (process.env.NODE_ENV !== 'production') {
            console.info("[legions:sandbox] " + this.name + " modified global properties restore...", __spreadArray([], __read(this.updatedValueSet.keys())));
        }
...
function getOverwrittenAppendChildOrInsertBefore(opts) {
    return function appendChildOrInsertBefore(newChild, refChild) {
        ...
        if (element.tagName) {
            ...
            var appName_1 = containerConfig.appName, appWrapperGetter = containerConfig.appWrapperGetter, proxy = containerConfig.proxy; containerConfig.strictGlobal; var dynamicStyleSheetElements = containerConfig.dynamicStyleSheetElements, scopedCSS = containerConfig.scopedCSS, excludeAssetFilter = containerConfig.excludeAssetFilter;
                ...
                case STYLE_TAG_NAME: {
                    ...
                    if (scopedCSS) {
                        ...
                        if (linkElementUsingStylesheet) {
                            stylesheetElement = convertLinkAsStyle(element, function (styleElement) { return process$1(mountDOM_1, styleElement, appName_1); }, fetch);
...
function patchLooseSandbox(appName, appWrapperGetter, proxy, mounting, scopedCSS, excludeAssetFilter) {
     ...
     */
    function () { return checkActivityFunctions(window.location).some(function (name) { return name === appName; }); }, function () { return ({
        appName: appName,
...
function hijack$2() {
    ...
    if (window.g_history &&
        isFunction(window.g_history.listen)) {
...
function hijack$1() {
    ...
    window.setInterval = function () {
        ...
        // @ts-ignore
        var intervalId = rawWindowInterval.apply(void 0, __spreadArray([], __read(args)));
    ...
    window.setTimeout = function () {
        ...
        // @ts-ignore
        var timerId = rawWindowTimeout.apply(void 0, __spreadArray([], __read(args)));
...
function hijack() {
    ...
    window.addEventListener = function (type, listener, options) {
        ...
        listenerMap.set(type, __spreadArray(__spreadArray([], __read(listeners)), [listener]));
...
function patchAtMounting(appName, elementGetter, sandbox, scopedCSS, excludeAssetFilter) {
    ...
    var patchersInSandbox = (_a = {},
        _a[SandBoxType.Proxy] = __spreadArray(__spreadArray([], __read(basePatchers)), [
            function () { return patchStrictSandbox(appName, elementGetter, sandbox.sandbox, true, scopedCSS, excludeAssetFilter); },
        ]),
        _a[SandBoxType.Snapshot] = __spreadArray(__spreadArray([], __read(basePatchers)), [
            ...
            return __awaiter(this, void 0, void 0, function () {
                ...
                return __generator(this, function (_a) {
                    ...
                    if (window.Proxy) {
                        ...
                        mountingFreers.push.apply(mountingFreers, __spreadArray([], __read(patchAtMounting(appName, elementGetter, sandbox, scopedCSS, excludeAssetFilter))));
                    }
                    /* ------------------------------------------ 3. 重置一些初始化时的副作用 ------------------------------------------*/
        ...
        unmount: function () {
            return __awaiter(this, void 0, void 0, function () {
                return __generator(this, function (_a) {
                    ...
                    sideEffectsRebuilders = __spreadArray(__spreadArray([], __read(bootstrappingFreers)), __read(mountingFreers)).map(function (free) { return free(); });
                    sandbox.inactive();
                    return [2 /*return*/];
                });
            });
...
function createElement(appContent, strictStyleIsolation, scopedCSS, appName) {
    ...
    if (scopedCSS) {
        ...
        forEach(styleNodes, function (stylesheetElement) {
        ...
        return __generator(this, function (_m) {
            switch (_m.label) {
                case 0:
                    ...
                    return [4 /*yield*/, importHTML(entry, importEntryOpts)];
                case 1:
                ...
                case 4:
                    ...
                    _d = mergeWith({}, getAddOns(global, assetPublicPath), lifeCycles, function (v1, v2) {
                        ...
                        var parcelConfig = {
                            ...
                            mount: [
                                ...
                                function (props) { return __awaiter(_this, void 0, void 0, function () {
                                    return __generator(this, function (_a) {
                                        return [2 /*return*/, mount(__assign(__assign({}, props), { container: appWrapperGetter(), setGlobalState: setGlobalState, onGlobalStateChange: onGlobalStateChange }))];
                                    });
                                }); },
...
function registerMicroApps(apps, lifeCycles) {
    ...
    unregisteredApps.forEach(function (app) {
        var name = app.name, activeRule = app.activeRule, _a = app.loader, loader = _a === void 0 ? noop : _a, props = app.props, _b = app.isMerge, isMerge = _b === void 0 ? false : _b, appConfig = __rest(app, ["name", "activeRule", "loader", "props", "isMerge"]);
        registerApplication({
            name: name,
                ...
                return __generator(this, function (_b) {
                    switch (_b.label) {
                        ...
                        case 2:
                            ...
                            return [2 /*return*/, __assign({ mount: __spreadArray(__spreadArray([
                                        function () { return __awaiter(_this, void 0, void 0, function () { return __generator(this, function (_a) {
                                            return [2 /*return*/, loader(true)];
                                        }); }); }
                                    ], __read(toArray(mount))), [
                                        function () { return __awaiter(_this, void 0, void 0, function () { return __generator(this, function (_a) {
...
function loadMicroApp(app, configuration, lifeCycles) {
    ...
    return mountRootParcel(memorizedLoadingFn, __assign({ domElement: document.createElement('div') }, props));
}
function start(opts) {
    if (opts === void 0) { opts = {}; }
    ...
    start$1({ urlRerouteOnly: urlRerouteOnly });
...
export { MicroApps$1 as MicroApps, MicroApps as MountedMicroApps, addGlobalUncaughtErrorHandler, getMicroAppStateActions, initGlobalState, loadMicroApp, prefetchImmediately as prefetchApps, registerMicroApps, removeGlobalUncaughtErrorHandler, runAfterFirstMounted, runDefaultMountEffects, setDefaultMountApp, start };
