...
> 已在内外经受过足够大量的线上系统的考验及打磨，健壮性值得信赖。
...
* 资源预加载，在浏览器空闲时间预加载未打开的子应用资源，加速子应用打开速度。


## Install

```
npm i legions-micro-service --save
```

## Usage

```js
import { registerMicroApps, start } from 'legions-micro-service';
...
registerMicroApps([
  {
    ...
    loader: ({ appContent }) => ReactDOM.render(<App appContent={appContent}>, document.getElementById('container')),
...
- 类型
  ...
  - `RegistrableApp`
    ...
    - entry - `string | { scripts?: string[]; styles?: string[]; html?: string }` - 必选，微应用的入口。
      - 配置为字符串时，表示微应用的访问地址，例如 `https://legions.umijs.org/guide/`。
...
- 示例
  ...
  ```tsx
  import { registerMicroApps } from 'legions-micro-service';
...
- 类型
  ...
  - `Options`
    ...
    - sandbox - `boolean` | `{ strictStyleIsolation?: boolean, experimentalStyleIsolation?: boolean }` - 可选，是否开启沙箱，默认为 `true`。
      ...
      默认情况下沙箱可以确保单实例场景子应用之间的样式隔离，但是无法确保主应用跟子应用、或者多实例场景的子应用样式隔离。当配置为 `{ strictStyleIsolation: true }` 时表示开启严格的样式隔离模式。这种模式下 legions 会为每个微应用的容器包裹上一个 [shadow dom](https://developer.mozilla.org/zh-CN/docs/Web/Web_Components/Using_shadow_DOM) 节点，从而确保微应用的样式不会对全局造成影响。

      <Alert>
        基于 ShadowDOM 的严格样式隔离并不是一个可以无脑使用的方案，大部分情况下都需要接入应用做一些适配后才能正常在 ShadowDOM 中运行起来（比如 react 场景下需要解决这些 <a target="_blank" href="https://github.com/facebook/react/issues/10422">问题</a>，使用者需要清楚开启了 <code>strictStyleIsolation</code> 意味着什么。后续 legions 会提供更多官方实践文档帮助用户能快速的将应用改造成可以运行在 ShadowDOM 环境的微应用。
      </Alert>
      ...
      div[data-legions-react16] .app-main {
        font-size: 14px;
      }
      ```
    ...
    - excludeAssetFilter - `(assetUrl: string) => boolean` - 可选，指定部分特殊的动态加载的微应用资源（css/js) 不被 legions 劫持处理。
...
- 示例
  ...
  ```ts
  import { start } from 'legions-micro-service';
...
- 示例
  ...
  ```ts
  import { setDefaultMountApp } from 'legions-micro-service';
...
- 示例
  ...
  ```ts
  import { runAfterFirstMounted } from 'legions-micro-service';
  ...
  class App extends React.Component {
    ...
    componentDidMount() {
      this.microApp = loadMicroApp(
        { name: 'app1', entry: '//localhost:1234', container: this.containerRef.current, props: { name: 'legions' } },
      );
    }
...
- 示例
  ...
  ```ts
  import { prefetchApps } from 'legions-micro-service';
...
- 示例
  ...
  ```ts
  import { addGlobalUncaughtErrorHandler } from 'legions-micro-service';
...
- 示例
  ...
  ```ts
  import { removeGlobalUncaughtErrorHandler } from 'legions-micro-service';
...
- 示例
  ...
  主应用：
  ```ts
  import { initGlobalState, MicroAppStateActions } from 'legions-micro-service';
